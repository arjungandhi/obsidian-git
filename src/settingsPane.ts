...
export class ObsidianGitSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    if (!this.plugin.gitReady) {
      containerEl.createEl("h4", {
        text:
          "Please clone or init a new repo to access the rest of the settings",
      });
    ...
    new Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.disablePush).onChange((value) => {
    ...
    new Setting(containerEl)
      ...
      .addText(async (cb) => {
        ...
        cb.onChange((value) =>
          this.plugin.gitManager.setConfig("user.name", value)
        );
      });
    ...
    new Setting(containerEl)
      ...
      .addText(async (cb) => {
        ...
        cb.onChange((value) =>
          this.plugin.gitManager.setConfig("user.email", value)
        );
      });
    ...
    new Setting(containerEl)
      ...
      .addText(async (cb) => {
        ...
        cb.setValue(
          await this.plugin.gitManager.getConfig("remote.origin.url")
        );
        cb.onChange(async (value) => {
          await this.plugin.gitManager.setConfig("remote.origin.url", value);
          this.plugin.gitManager.setConfig(
            "remote.origin.fetch",
  ...
  addCloneButton() {
    ...
    setting.setName("Repository URL").addText((cb) => {
      cb.inputEl.autocapitalize = "off";
      cb.inputEl.autocomplete = "off";
      cb.inputEl.spellcheck = false;
  ...
  addStandaloneModeToggle() {
    new Setting(this.containerEl)
      ...
      .setDesc(
        "No system wide git installation is needed. See README for limitations and instructions."
      )
      .addToggle((cb) =>
        cb.setValue(this.plugin.settings.standaloneMode).onChange((value) => {
          if ((this.app as any).isMobile) {
  ...
  addStandaloneSettings() {
    ...
    new Setting(containerEl).setName("Username").addText((cb) => {
      cb.setValue(
        window.localStorage.getItem(this.plugin.manifest.id + ":username")
      );
      cb.onChange((value) =>
    ...
    const passwordSetting = new Setting(containerEl)
      ...
      .setDesc(
        "Type in your password and press on the button to set it. You won't be able to see it again."
      )
      .addText((cb) => {
        cb.inputEl.autocapitalize = "off";
        cb.inputEl.autocomplete = "off";
