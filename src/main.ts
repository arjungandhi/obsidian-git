...
export default class ObsidianGit extends Plugin {
  ...
  async init(): Promise<void> {
    if (this.settings.standaloneMode || (this.app as any).isMobile) {
      ...
      this.displayError(
        "This wont work on desktop you have your .obsidian folder set incorrectly"
      );
    }
    // else {
    ...
    switch (result) {
      ...
      case "missing-repo":
        new Notice(
          "Can't find a valid git repository. Please go to the settings"
        );
        break;
      case "wrong-settings":
        this.displayError(
          "Not all of the required standalone mode settings are set"
        );
      case "valid":
        this.gitReady = true;
